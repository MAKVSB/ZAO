...
for dataset in glob("./../lec7/datasets/*", recursive = True):
    for imagePath in glob(dataset + "/*.jpg", recursive = True):
        ...
        for face in faces:
            ...
            _, landmarks = landmark_detector.fit(imageDataBW, np.array([face]))
            ...
            mask = mask[face[1]:face[1] + face[3], face[0]:face[0] + face[2]]

            output3 = cv2.seamlessClone(resized_image, imageData, mask, center_face, cv2.MONOCHROME_TRANSFER)
